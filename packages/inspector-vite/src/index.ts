...
import type { PersistedInspectorState } from "../../inspector/src/types";
    ...
    configureServer(server) {
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        if (req.method === "POST") {
          ...
          const parsed = JSON.parse(body) as { state?: PersistedInspectorState };
          if (parsed.state) {
            await writeJsonFile(filePath, parsed.state);
          }
          res.statusCode = 204;
          res.end();
          return;
